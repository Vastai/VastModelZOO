...
2. 基于[image2npz.py](../../common/utils/image2npz.py)，将评估数据集转换为npz格式，生成对应的`npz_datalist.txt`：
    ...
    ```
3. 性能测试，配置vdsp参数[salod-scrn-vdsp_params.json](../vacc_code/vdsp_params/salod-scrn-vdsp_params.json)，执行：
    ```bash
    vamp -m deploy_weights/scrn-int8-kl_divergence-3_512_512-vacc/scrn \
    --vdsp_params vacc_code/vdsp_params/salod-scrn-vdsp_params.json \
    -i 1 p 1 -b 1
    ```
4. 精度测试，推理得到npz结果：
    ```bash
