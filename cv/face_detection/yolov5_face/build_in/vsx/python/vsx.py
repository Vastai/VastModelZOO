...
class FaceDetector:
    ...
    def post_processing(self, input_id, height, width, stream_output_list):
        ...
        shape = [(1, 48, 80, 80), (1, 48, 40, 40), (1, 48, 20, 20)]

        for i in range(3):
            yolo_layer = torch.Tensor(stream_ouput_data[i].reshape(shape[i]))
        ...
        for i, det in enumerate(pred):  # detections per image
            if len(det):
                ...
                det[:, :4] = scale_coords([640, 640], det[:, :4], (height, width, 3))#.round()
                    ...
                    self.result_dict[input_id].append(
                        {
                            ...
                            "label": 'face',
                            "bbox": box,
                            "score": float(conf if conf <= 1 else 1),
                        }
                    )
