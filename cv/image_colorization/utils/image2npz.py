...
def make_npz_text(args):
    ...
    with open(args.text_path, 'w') as f:
        ...
        for img_file in tqdm(files_list):
            img_data = cv2.imread(img_file, 1)
...
if __name__ == "__main__":
    ...
    parse.add_argument("--dataset_path", type=str, default="/path/to/dataset/face/GPEN/hq_gray", help="path to input source image folder")
    parse.add_argument("--target_path", type=str, default="/path/to/dataset/face/GPEN/hq_gray_npz", help="path to output npz image folder")
    parse.add_argument("--text_path", type=str, default="datalist_npz_gray.txt")
