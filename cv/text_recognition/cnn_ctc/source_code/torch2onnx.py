...
def parse_args():
    ...
    parser.add_argument(
        "--dummy_input_shape", type=str, default="3,32,100", help="input shape (e.g. 3,32,100) in C,H,W format"
    )
    parser.add_argument("--dynamic_shape", action="store_true", help="whether to use dynamic shape")
    parser.add_argument("--opset_version", default=10, type=int, help="onnx opset version")
    parser.add_argument(
        "--do_constant_folding", action="store_true", help="whether to apply constant-folding optimization"
    )
    parser.add_argument("--verbose", action="store_true", help="whether print convert info")
...
def main():
    ...
    print(dummy_input[0].shape,dummy_input)
    ...
    if args.dynamic_shape:
        print(f"Convert to Onnx with dynamic input shape and opset version" f"{args.opset_version}")
    else:
        print(
    ...
    torch.onnx.export(model, dummy_input, args.out, input_names=["input"])
    runner.logger.info(f"Convert successfully, saved onnx file: {os.path.abspath(args.out)}")
