...
if __name__ == '__main__':
    ...
    with open(args.output_file, "w") as outfile:
        for img_path in tqdm.tqdm(images, desc="Evaluating"):
            ...
            pred_idx = int(features.argmax())
            pred_label = label_list[pred_idx]
            score = float(features[pred_idx])
            if pred_idx == gt_label:
                correct += 1

            decode_out = (pred_label, score)
            print(f"{img_path}: VSX={decode_out}")
            outfile.write(f"{img_path} VSX={decode_out}\n")
