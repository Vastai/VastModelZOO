...
parser.add_argument("--file_path", type=str, default="datasets/text_image_orientation", help="Dataset root (joined with relative paths in label_file)")
...
if __name__ == '__main__':
    ...
    with open(args.output_file, "w") as outfile:
        for img_path in tqdm.tqdm(images, desc="Evaluating"):
            ...
            if use_onnx:
                ...
                all_onnx_preds.append(logits)
                onnx_idx = logits.argmax(axis=1)
                onnx_out = [(label_list[idx], float(logits[0, idx])) for idx in onnx_idx]
                parts.append(f"ONNX={onnx_out}")

            print(f"{basename}: {', '.join(parts)}")
            outfile.write(f"{basename} {' '.join(parts)}\n")
    ...
    if len(all_labels) > 0:
        ...
        print("\n========== Top-1 Accuracy ==========")
        if use_onnx:
            all_onnx_preds = np.concatenate(all_onnx_preds, axis=0)
            onnx_acc = topk_accuracy(all_onnx_preds, all_labels, topk=(1,))
            print(f"ONNX  Top-1: {onnx_acc[1]:.2f}%")
