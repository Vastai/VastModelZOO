...
- 执行以下命令获取 `onnx`/`torchscript` 模型:
    ```bash
    cd /path/to/yolov5
    python export.py --weights /path/to/yolov5m-cls.pt --include onnx torchscript --img 224
    ```
- 在原模型路径下可获取转换后的模型权重
...
- 精度评估：[eval_topk.py](../common/eval/eval_topk.py)
    ...
    ```
    # official_yolov5m_cls_fp16 224

    ## fp16
    top1_rate: 75.854 top5_rate: 92.814

    ## int8
    top1_rate: 75.310 top5_rate: 92.572
    ```
    </details>
