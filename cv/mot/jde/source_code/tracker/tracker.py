...
from vamc.loader import DataIterator, get_activation_aligned
...
class JDETracker(object):
    def __init__(self, opt, weights_dir, model_name, frame_rate=30):
        self.opt = opt
    ...
    def _get_model(self, weights_dir, model_name):

        hash = hashlib.md5()
        hash.update(model_name.encode())
        md5_model = hash.hexdigest()
        model_key = f"{md5_model}::{model_name}"
        kwargs =  {"name": model_key}

        # 指定die
        ctx = tvm.vacc(0)
        loaded_json = open(f"{weights_dir}/{model_name}.json").read()
        loaded_lib = tvm.module.load(f"{weights_dir}/{model_name}.so")
        loaded_params = bytearray(open(f"{weights_dir}/{model_name}.params", "rb").read())
    ...
    def update(self, im_blob, img0):
        ...
        input_image = get_activation_aligned(im_blob.numpy().astype("float16"))
        self.model.set_input(self.name, "input", 0, tvm.nd.array(input_image))

        self.model.run(self.name)

        layer0 = torch.Tensor(self.model.get_output(self.name, 0).asnumpy())
        layer1  = torch.Tensor(self.model.get_output(self.name, 1).asnumpy())
        layer2  = torch.Tensor(self.model.get_output(self.name, 2).asnumpy())
        layer3 = torch.Tensor(self.model.get_output(self.name, 3).asnumpy())
        layer4  = torch.Tensor(self.model.get_output(self.name, 4).asnumpy())
        layer5  = torch.Tensor(self.model.get_output(self.name, 5).asnumpy())
