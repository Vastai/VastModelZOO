...
class VSXInference:
    ...
    def _run(self, image:Union[str, np.ndarray]):
        if isinstance(image, str):
            ...
            image = np.stack(cv2.split(transformed_image))
...
def set_config():
    ...
    parse.add_argument(
        "--lr_image_dir",
        type=str,
        default="/path/to/CelebA/Img/img_align_celeba",
    ...
    parse.add_argument(
        "--model_prefix_path",
        type=str,
        default="deploy_weights/official_pfsrnet_run_stream_fp16/mod",
        help="model info")
    parse.add_argument(
        "--vdsp_params_info",
        type=str,
        default="../vacc_code/vdsp_params/official-pfsrnet-vdsp_params.json",
        help="vdsp op info",
    )
...
if __name__ == '__main__':
    ...
    # iamge_files = image_files[:1000]
    # print(len(iamge_files))
    ...
    results = vsx_inference.run_batch(image_files[:1000])
    #############################################################################
    # import onnxruntime
    ...
    ssim_list = []
    # results = image_files
    for (image_path, result) in tzip(image_files[:1000], results):
        #############################################################################
