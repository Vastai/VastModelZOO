...
class VSXInference:
    ...
    def _run(self, image:Union[str, np.ndarray]):
        if isinstance(image, str):
            ...
            image = np.stack(cv2.split(image))
...
def set_config():
    ...
    parse.add_argument(
        "--lr_image_dir",
        type=str,
        default="./dataset/sr/CelebAMask-HQ/test_img",
    ...
    parse.add_argument(
        "--model_prefix_path",
        type=str,
        default="deploy_weights/fsrnet-fp16-none-3_128_128-vacc/mod",
        help="model info")
    parse.add_argument(
        "--vdsp_params_info",
        type=str,
        default="super_resolution/fsrnet/vacc_code/vdsp_params/official-fsrnet-vdsp_params.json",
        help="vdsp op info",
    )
...
if __name__ == '__main__':
    ...
    image_files = glob.glob(os.path.join(args.lr_image_dir, "*.jpg"))
    # sort images
    # image_files.sort()
    ...
    for (image_path, result) in tzip(image_files, results):
        ...
        sr_image = _denormalize(output)[0].astype('uint8')
        if sr_image.shape[0] != input_shape:
            sr_image = cv2.resize(sr_image, input_shape)

        # draws
        cv2.imwrite(os.path.join(args.save_dir, os.path.basename(image_path)), sr_image[:,:,::-1])

        # eval
        image_src = Image.open(image_path)
        image_hr = image_src.resize(input_shape, Image.BICUBIC)
        image_hr = np.array(image_hr)
        from skimage.metrics import peak_signal_noise_ratio as compare_psnr
        from skimage.metrics import structural_similarity as compare_ssim
        psnr = compare_psnr(image_hr, sr_image)
        ssim = compare_ssim(image_hr, sr_image, channel_axis = -1)
        # ssim = calculate_ssim(image_hr, out)
        psnr_list.append(psnr)
        ssim_list.append(ssim)
        print("{} ---> psnr:{}, ssim:{}".format(image_path, psnr, ssim))
