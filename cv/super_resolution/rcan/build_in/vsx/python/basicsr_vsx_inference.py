...
def set_config():
    ...
    parse.add_argument(
        "--lr_image_dir",
        type=str,
        default="/path/to/DIV2K_valid_LR_bicubic/X2",
    ...
    parse.add_argument(
        "--model_prefix_path",
        type=str,
        default="deploy_weights/basicsr_rcan_run_stream_int8/mod",
        help="model info")
    parse.add_argument(
        "--vdsp_params_info",
        type=str,
        default="../vacc_code/vdsp_params/basicsr-rcan-vdsp_params.json",
        help="vdsp op info",
    )
...
if __name__ == '__main__':
    ...
    # model  = torch.jit.load("deploy_weights/model_best.onnx.torchscript.pt", map_location=device)
    ...
    for (image_path, result) in tzip(image_files, results):
        ...
        # image = cv2.resize(image_lr, (1920, 1080))
        ...
        # result = np.clip(heatmap * 255.0, 0, 255)
        ...
        # eval
        image_gt = cv2.imread(os.path.join(args.hr_image_dir, os.path.basename(image_path).replace("x2", ""))) # set5 use ("LRBI", "HR") # div2k use ("x2", "")
...
vacc int8 mean psnr: 29.469629865749102, mean ssim: 0.7310496635228203
torch     mean psnr: 29.551427980242025, mean ssim: 0.7381508893487269
"""