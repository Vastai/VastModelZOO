...
if __name__ == "__main__":
    ...
    parse.add_argument("--gt_dir", type=str, default="/path/to/code/model_check/SR/BasicSR/datasets/DIV2K/DIV2K_valid_HR", help="hr gt image folder")
    parse.add_argument("--input_npz_path", type=str, default="/path/to/code/model_check/SR/BasicSR/datasets/DIV2K/DIV2K_valid_LR_bicubic/npz_datalist_X2.txt", help="npz text file")
    parse.add_argument("--out_npz_dir", type=str, default="/path/to/code/vamc/vamp/0.2.0/outputs/lesrcnn", help="vamp output folder")
    ...
    with open(args.input_npz_path, 'r') as f:
        ...
        for i, line in enumerate(tqdm(file_lines)):
            ...
            image_gt = cv2.imread(os.path.join(args.gt_dir, os.path.basename(file_name).replace("x2", ""))) # div2k use ("x2", "")
...
"""
runstream比runmodel精度更低,应该和resize_type有关, 如输入先进行了image = cv2.resize(image, [128, 128], interpolation=cv2.INTER_AREA), 精度可对齐

lesrcnn_x2-int8-kl_divergence-3_128_128-vacc
mean psnr: 20.6615370966972, mean ssim: 0.6259584348565821
