...
def argument_parser():
    ...
    parser.add_argument(
        '--input_file',
        default=
        '/opt/vastai/vaststreamx/data/datasets/fov_pointcloud_float16/000001.bin',
        help='input file',
    )
    parser.add_argument(
        '--save_npz',
...
if __name__ == '__main__':
    ...
    filelist = [
        f
        for f in sorted(glob.glob(args.dataset_root +
                                  '/*.bin', recursive=True))
    ]
    for file in tqdm(filelist):
        ...
        np.savez(
            os.path.join(args.dataset_output_folder,
