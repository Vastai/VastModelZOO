...
class CenterPoint(CustomOpBase):
    def __init__(
        self,
        model_configs,
        elf_file,
        ...
        super().__init__(op_name=pointpillar_op_name,
                         elf_file=elf_file,
                         device_id=device_id)
        ...
        for i in range(len(model_configs)):
            model = vsx.Model(model_configs[i].model_prefix, batch_size,
                              model_configs[i].hw_config)
        ...
        np_array = np.random.randint(low=0,
                                     high=256,
                                     size=(fix_buffer_size),
                                     dtype=np.uint8)
        self.tmp_buffer_tensor_ = vsx.from_numpy(np_array, self.device_id_)
        np_array = np.random.randint(low=0,
                                     high=256,
                                     size=(PP_MODEL_OUTPUT_SIZE),
                                     dtype=np.uint8)
        ...
        print(f'fix_buffer_size={fix_buffer_size}')
    ...
    def get_test_data(self, dtype, input_shape, batch_size, ctx='VACC'):
        if ctx == 'CPU':
            ...
            return [
                vsx.from_numpy(np.zeros(input_shape, dtype=dtype),
                               self.device_id_)
            ] * batch_size

    def process(
        self,
        input: Union[List[List[np.ndarray]], List[List[vsx.Tensor]],
                     List[np.ndarray], List[vsx.Tensor], ],
    ):
        if isinstance(input, list):
            if isinstance(input[0], np.ndarray):
                return self.process(
                    [vsx.from_numpy(x, self.device_id_) for x in input])
            else:
    ...
    def process_impl(self, inputs):
        ...
        for input in inputs:
            ...
            self.custom_op_.run_sync(
                tensors=[input],
                config=ctypes.string_at(ctypes.byref(self.op_conf_),
                                        op_conf_size),
                output_info=[
