...
class DemoDataset(DatasetTemplate):
    def __init__(
        ...
        data_file_list = (glob.glob(str(root_path / f'*{self.ext}'))
                          if self.root_path.is_dir() else [self.root_path])
    ...
    def __getitem__(self, index):
        if self.ext == '.bin':
            print('self.sample_file_list[index]: {name}'.format(
                name=self.sample_file_list[index]))
            id_num = int(self.sample_file_list[index].split('/')[-1].split('.')
                         [0].split('_')[-1])
            if (id_num == 1200):
                print()
            ...
            if intensity.max() > 1.0 or intensity.min() < 0.0:
                ...
                points[:,
                       -1] = (intensity - intensity.min()) / (intensity.max() -
                                                              intensity.min())
            print()
        elif self.ext == '.npy':
...
def main():
    ...
    if export_onnx:
        ...
        dummy_input = torch.randn(args.max_voxel_num, 32,
                                  10).cuda()  # 根据您的模型输入形状替换这行 6445
        onnx_file_path = os.path.join(args.save_path,
                                      'PillarVFE.onnx')  # 自定义ONNX输出路径
        torch.onnx.export(
            model,
            dummy_input,
            onnx_file_path,
        ...
        dummy_input = torch.randn(
            args.backbone_input_shape[0], args.backbone_input_shape[1],
            args.backbone_input_shape[2],
            args.backbone_input_shape[2]).cuda()  # 根据您的模型输入形状替换这行
        onnx_file_path = os.path.join(
            args.save_path, 'BaseBEVBackbone_CenterHead.onnx')  # 自定义ONNX输出路径
        torch.onnx.export(
            model,
            dummy_input,
            onnx_file_path,
