...
def symbolic(g, pillar_features, coords, mask, size_x, size_y, size_z,
             size_features):
    return g.op(
...
def parse_config():
    ...
    parser.add_argument(
        ...
        type=str,
        default=
        './project/vehicle-road-collaboration-lance_det3d/model_centerpoint_pp.yaml',
    ...
    parser.add_argument(
        ...
        type=str,
        default=
        './project/vehicle-road-collaboration-lance_det3d/Zhongxing_dense_crowd_v1/lidar_data_fp32_332.bin',
    ...
    parser.add_argument(
        ...
        type=str,
        default=
        './project/vehicle-road-collaboration-lance_det3d/checkpoint_epoch_126.pth',
...
def main():
    ...
    with torch.no_grad():
        ...
        dummy_voxels = torch.zeros((MAX_VOXELS, 32, 10),
                                   dtype=torch.float32,
                                   device='cpu')
        ...
        dummy_mask = torch.ones((1, MAX_VOXELS),
                                dtype=torch.int8,
                                device='cpu')
        ...
        torch.onnx.export(
            ...
            # custom_opsets = 10
            input_names=['voxels', 'voxel_coords',
                         'mask'],  # the model's input names
            # input_names = ['spatial_features',],
            output_names=['cls_preds', 'box_preds', 'dir_cls_preds'
                          ],  # the model's output names, 置信度，box,方向分类(前或后)
        )
    print('export ok')
    exit(0)
