...
def make_scatterND(model, rpn_input_shape, indices_shape, mask_shape, pfe_out_maxpool_name, batch_size, save_for_trt=False):
    ...
    if save_for_trt:
        ...
        data_shape = [batch_size, rpn_input_shape[1], rpn_input_shape[2], rpn_input_shape[3]]
        data = np.zeros(data_shape, dtype=np.float32)
        data_tensor = numpy_helper.from_array(data, name="scatter_data")
        model.graph.initializer.append(data_tensor)
    ...
    if save_for_trt:
        ...
        input_node = onnx.helper.make_tensor_value_info('voxel_coords_xyz', idx_type, [indices_shape[1], indices_shape[2]])
        model.graph.input.append(input_node)
        mask_type = onnx.TensorProto.INT8
        mask_node = onnx.helper.make_tensor_value_info('mask', mask_type, [1, mask_shape[1]])
...
parser.add_argument("--pfe_model_path", type = str, default = "./onnx_models/PillarVFE_32000_sim.onnx", help = "pfe model path")
parser.add_argument("--rpn_model_path", type = str, default = "./onnx_models/BaseBEVBackbone_CenterHead_sim.onnx", help="rpn model path")
parser.add_argument("--target_model_path", type = str, default = "./onnx_models/", help="target model path")
parser.add_argument("--target_model_name", type = str, default = "model_centerpoint_pp_32000_v5.onnx", help="target model path")
parser.add_argument('--max_voxel_num',type=int,default='32000',help='max voxel num')
parser.add_argument("--backbone_input_shape",type=parse_int_list,default = [1,64,480,480], help = "backbone input shape")
...
if __name__ == "__main__":
    ...
    pointpillars_save_path = os.path.join(args.target_model_path, args.target_model_name)
    ...
    rpn_input_shape = [batch_size,args.backbone_input_shape[1],args.backbone_input_shape[2],args.backbone_input_shape[3]]
    ...
    pfe_model.graph.input[0].type.tensor_type.shape.dim[0].dim_value = points_num
    pfe_model.graph.output[0].type.tensor_type.shape.dim[0].dim_value = points_num
    ...
    make_scatterND(pfe_model, rpn_input_shape, indices_shape, mask_shape, pfe_out_Squeeze_name, batch_size)
