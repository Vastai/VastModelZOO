...
2. 模型转换配置文件
    - [official_kitti_center_point.yaml](../vacc_code/build/official_kitti_center_point.yaml)
    ...
    mkdir workspace
    cd workspace
    vamc compile ../vacc_code/build/official_kitti_center_point.yaml
    ```
...
1. 参考[vsx/center_point_runstream.py](../vacc_code/vsx/center_point_runstream.py)：生成推理的txt结果
    ```bash
...
1. 运行时backbone_input_shape参数计算公式
    ...
    伪图尺寸W = (69.12 - 0) / 0.16  = 432
    伪图尺寸H= (39.68 - (-39.68)) / 0.16 = 496
    ```

2. 由于我们把点云数据预处理也使用dsp算子进行计算，所以对于最大voxel的数量有一定的限制，需要小于16M。
    - 最大体素数(max_voxel_num) x 各体素最大点数(max_points_per_voxel) * 每个点特征所占字节(每个点云4个特征xyz及强度, 每个特征为fp16, 为8)

3. 由于VACC部署center_point模型需要把bevfusion后的伪图放到SSRAM中，才能加速运算。因此，伪图所占用的空间是有限制的。
    - 计算公式为：伪图宽 x 伪图高 x 通道数 x 数据类型所占字节 < 40M SSRAM
    - 另外考虑f16必须大core这个40M可能还得砍半
    - 在实际应用中，我们需要记住这样的限制条件，否则会导致模型无法运行。