...
def set_config():
    ...
    parse.add_argument(
        "--file_path",
        type=str,
        default="/path/to/carvana/imgs",
        help="img dir",
    )
    parse.add_argument(
        "--gt_path",
        type=str,
        default="/path/to/carvana/masks",
        help="mask_dir",
    )
    parse.add_argument(
        "--model_prefix_path",
...
if __name__ == '__main__':
    ...
    for (image_path, result) in tzip(image_files, results):
        ...
        # draw
        heatmap = torch.from_numpy(result).unsqueeze(0)
        predict = heatmap.max(dim=1)[1].cpu().numpy()
        mask = predict * 255
        ...
        ########################################################################################################
        # eval
        label_path = os.path.join(args.gt_path, os.path.basename(image_path).replace(".jpg", "_mask.gif"))
