...
def testGenerator(test_path,num_image = 30,target_size = (256,256),flag_multi_class = False,as_gray = False):
    ...
    #for pngfile in glob.glob(test_path + "/*.png"):#查找有关目录下的所有文件，pngfile为路径
        img = cv2.imread(os.path.join(test_path,"%d.png"%i))#在OpenCV第一个参数是指定需要读取的图片的路径和图片名，另一个参数，常用的就是"IMREAD_UNCHANGED"、"IMREAD_GRAYSCALE"、"IMREAD_COLOR"三个属性
...
def saveResult(save_path,npyfile,flag_multi_class = False,num_class = 2):
    ...
    #     cv2.normalize(img, img, 0, 255, cv2.NORM_MINMAX)  # 使用opencvimwrite保存图片时，需做归一化，将原先归一化至0-1的图片复原出来，否则结果全黑
    #     img = np.array(img)
    #     cv2.imwrite(save_path + "/" + str(i+1) + "_predict.png", img)
