...
def set_config():
    ...
    parse.add_argument(
        "--model_prefix_path",
        type=str,
        default="deploy_weights/official_deeplab_v3_run_stream_int8/mod",
        help="model info")
    parse.add_argument(
        "--vdsp_params_info",
        type=str,
        default="../vacc_code/vdsp_params/vainf-deeplab_v3_resnet50-vdsp_params.json",
        help="vdsp op info",
    )
...
if __name__ == '__main__':
    ...
    # model  = torch.jit.load("deeplab_v3_resnet50-513.torchscript.pt", map_location=device)
    ...
    for (image_path, result) in tzip(image_files, results):
        ...
        gt = gt.resize(size=heatmap.shape[-2:][::-1])
