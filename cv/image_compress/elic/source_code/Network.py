...
class TestModel(CompressionModel):
    ...
    def compress(self, x):
        onnx_out_path = "./compress_onnx_output"
        os.makedirs(onnx_out_path, exist_ok=True)
        ...
        # to export ga onnx model
        ga_onnx = os.path.join(onnx_out_path, "ga.onnx")
        torch.onnx.export(self.g_a, 
                            ...
                            verbose=True)           # verbose (可选): 是否打印详细的信息, 默认为 False

        # to sim ga onnx model
        from onnxsim import simplify
        import onnx
        path_onnx_sim = ga_onnx.replace('.onnx', '_sim.onnx')
        model_onnx = onnx.load(ga_onnx)
        model_simplified, check = simplify(model_onnx)
        assert check, "Simplified ONNX model could not be validated."
        onnx.save(model_simplified, path_onnx_sim)

        # to export ha onnx model
        ha_onnx = os.path.join(onnx_out_path, "ha.onnx")
        ...
        # to sim ha onnx model
        from onnxsim import simplify
        import onnx
        path_onnx_sim = ha_onnx.replace('.onnx', '_sim.onnx')
        model_onnx = onnx.load(ha_onnx)
        model_simplified, check = simplify(model_onnx)
        assert check, "Simplified ONNX model could not be validated."
        onnx.save(model_simplified, path_onnx_sim)
    ...
    def decompress(self, strings, shape):
        onnx_out_path = "./decompress_onnx_output"
        os.makedirs(onnx_out_path, exist_ok=True)
        ...
        # to export hs onnx model
        hs_onnx = os.path.join(onnx_out_path, "hs_chunk.onnx")
        self.h_s.eval()
        torch.onnx.export(self.h_s, 
                          ...
                          export_params=True, 
                          verbose=True)           
        # to sim hs onnx model
        from onnxsim import simplify
        import onnx
        path_onnx_sim = hs_onnx.replace('.onnx', '_sim.onnx')
        model_onnx = onnx.load(hs_onnx)
        model_simplified, check = simplify(model_onnx)
        assert check, "Simplified ONNX model could not be validated."
        onnx.save(model_simplified, path_onnx_sim)
        ...
        # to export gs onnx model
        gs_onnx = os.path.join(onnx_out_path, "gs.onnx")
        self.g_s.eval()
        torch.onnx.export(self.g_s, 
                            y_hat,                  # 一个或多个张量，表示模型的输入, 多个用 tuple 表示
                            gs_onnx, 
                            input_names=['y_hat'],
                            output_names=['x_hat'],
                            opset_version=11,
        ...
        # to sim gs onnx model
        from onnxsim import simplify
        import onnx
        path_onnx_sim = gs_onnx.replace('.onnx', '_sim.onnx')
        model_onnx = onnx.load(gs_onnx)
        model_simplified, check = simplify(model_onnx)
        assert check, "Simplified ONNX model could not be validated."
        onnx.save(model_simplified, path_onnx_sim)
