...
from elic_base import VastaiGs, VastaiHsChunk
...
class ElicDecompress(CompressionModel):
    def __init__(
        self,
        hs_model_prefix,
        gs_model_prefix,
        ...
        self.vastai_g_s = VastaiGs(
            gs_model_prefix, tensorize_elf_path, batch_size, device_id, gs_hw_config
        )
    ...
    def decompress(self, strings, shape):
        ...
        latents = self.vastai_h_s.process(z_hat.numpy().astype("float16"))
        latent_means = torch.from_numpy(latents[0])
        latent_scales = torch.from_numpy(latents[1])
