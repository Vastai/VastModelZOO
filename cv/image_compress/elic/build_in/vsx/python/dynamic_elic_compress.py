...
from elic_dynamic_base import VastaiDynamicGaHa, VastaiDynamicHsChunk

# from common.vastai_elic import VastaiHsChunk
...
class DynamicElicCompress(CompressionModel):
    def __init__(
        self,
        gaha_model_info,
        gaha_vdsp_config,
        ...
        #     gaha_model_prefix, gaha_vdsp_config, batch_size, device_id, gaha_hw_config
        # )
        self.vastai_dynamic_gaha = VastaiDynamicGaHa(
            gaha_model_info,
            gaha_vdsp_config,
            max_input_size,
            batch_size,
        ...
        # self.vastai_hs_chunk = VastaiHsChunk(
        #     hs_model_prefix, batch_size, device_id, hs_hw_config
        # )
        # TODO. gaha_max_input_size -> hs_max_input_size
        self.vastai_dynamic_hs = VastaiDynamicHsChunk(
            hs_model_info, [[1, 192, 32, 32]], batch_size, device_id
        )
    ...
    def get_fusion_op_iimage_format(self):
        ...
        return self.vastai_dynamic_gaha.get_fusion_op_iimage_format()

    def process(self, input: Union[np.ndarray, vsx.Image]):
        # y, z = self.vastai_ga_ha.process(input)
        y, z = self.vastai_dynamic_gaha.process(input)
        ...
        ctx_params_anchor_split = torch.split(
            torch.zeros(B, C * 2, H, W), [2 * i for i in self.groups[1:]], 1
        )
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            y_anchor_encode = torch.zeros(B_anchor, C_anchor, H_anchor, W_anchor // 2)
