...
def save_result(image_path, result, save_dir):
    ...
    heatmap = np.transpose(features, (1,2,0))
    heatmap = cv.resize(heatmap, (0,0), fx=8, fy=8, interpolation=cv.INTER_CUBIC)
    heatmap = cv.resize(heatmap, (origin_img.shape[1], origin_img.shape[0]), interpolation=cv.INTER_CUBIC)
    
    

    file_name = 'heatmap_' + os.path.split(image_path)[-1]
    # cv.imwrite(os.path.join(save_dir, file_name), heatmap)
