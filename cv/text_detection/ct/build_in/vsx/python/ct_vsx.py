...
parse.add_argument(
    "--file_path",
    type=str,
    default= "/path/to/total_text_ppocr/test/rgb/",
...
parse.add_argument(
    "--vdsp_params_info",
    type=str,
    default="../vacc_code/vdsp_params/vdsp_params.json", 
    help="vdsp op info",
)
parse.add_argument(
    "--label_txt", type=str, default="/path/to/total_text_ppocr/test/test_ppocr.txt", help="label txt"
)
...
if __name__ == '__main__':
    detector = CTDetector(model_prefix_path=args.model_prefix_path,
    ...
    if os.path.isfile(args.file_path):
            ...
            for line in tqdm(f.readlines(), desc='calc metric'):
                ...
                for lable_str_ in lable_str:
                    ...
                    if lable_str_["transcription"] == '###':
                        continue
                    text_.append(lable_str_["transcription"])
                    label_.append(np.array(lable_str_["points"]).reshape((1, -1)))
                label[file_name] = label_
                text[file_name] = text_
        ...
        for (image, result) in zip(images, results):
            ...
            shape_list = [list(ori_image.shape) + [640, 960, 3]]
            post_result = postprocess_op(result[0], shape_list)[0]
            eval_class(post_result, [label[image.split('/')[-1]], text[image.split('/')[-1]]])
