...
class EASTDetector:
    ...
    def post_processing(self, input_id, stream_output_list):
        ...
        geo_list = stream_output_list[0][0].astype(np.float32)
        score_list = stream_output_list[0][1].astype(np.float32)

        self.result_dict[input_id].append(np.expand_dims(score_list, 0))
        self.result_dict[input_id].append(np.expand_dims(geo_list, 0))
...
if __name__ == '__main__':
    detector = EASTDetector(model_prefix_path=args.model_prefix_path,
    ...
    if os.path.isfile(args.file_path):
        ...
        for (image, result) in zip(images, results):
            ...
            shape_list = np.array([[720, 1280, float(704) / 720, float(1280) / 1280]])
            score_list = result[0]
            geo_list =  result[1]
            post_result = postprocess_op(score_list, geo_list, shape_list)[0]
            #新的编译器尺寸变了
