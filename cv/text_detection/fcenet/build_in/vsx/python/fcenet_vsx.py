...
class FCENetDetector:
    ...
    def post_processing(self, input_id, stream_output_list):
        ...
        preds = stream_output_list[0]#.astype(np.float32)
        # print(preds.shape)

        self.result_dict[input_id].append(preds)
    ...
    def _run(self, image: Union[str, np.ndarray]):
        if isinstance(image, str):
            ...
            image = np.stack(cv.split(cv_image))
    ...
    def run_sync(self, image: Union[str, np.ndarray]):
        ...
        model_output_list = [vsx.as_numpy(out).astype(
...
if __name__ == '__main__':
    detector = FCENetDetector(model_prefix_path=args.model_prefix_path,
    ...
    if os.path.isfile(args.file_path):
        ...
        for (image, result) in zip(images, results):
            ...
            post_result = postprocess_op(cls, reg, shape_list)[0]
