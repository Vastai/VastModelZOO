...
from source_code.mmocr.db_postprocess import DBPostProcess
...
parse.add_argument("--model_prefix_path", type=str,
                   default="./models/", help="model info")
parse.add_argument(
    "--vdsp_params_info",
    type=str,
...
class DBNetDetector:
    ...
    def _run(self, image: Union[str, np.ndarray]):
        ...
        nv12_image = vsx.create_image(
            image, vsx.ImageFormat.RGB_PLANAR, width, height, self.device)
...
if __name__ == '__main__':
    ...
    if os.path.isfile(args.file_path):
        ...
        for (image, result) in zip(images, results):
            ...
            shape_list = np.array(
                [[720, 1280, float(736) / 720, float(1280) / 1280]])
            heatmap = result[0].squeeze(0)
            heatmap = sigmoid(heatmap)
            post_result = postprocess_op(heatmap, shape_list)[0]
            eval_class(post_result, label[image.split(
                '/')[-1]], ignore[image.split('/')[-1]])
...
# fp16
metric:  {'precision': 0.8972562755399883, 'recall': 0.7400096292729899, 'hmean': 0.8110817941952507}

# int8-percentile
metric:  {'precision': 0.8640832851359167, 'recall': 0.719306692344728, 'hmean': 0.7850761954808196}

# int8-kl 
metric:  {'precision': 0.8715647784632642, 'recall': 0.7481945113143957, 'hmean': 0.805181347150259}

# int8-max
metric:  {'precision': 0.8597081930415263, 'recall': 0.7376023110255175, 'hmean': 0.7939880798134231}
'''