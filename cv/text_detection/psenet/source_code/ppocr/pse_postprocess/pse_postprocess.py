...
from ppocr.pse_postprocess.pse import pse
...
class PSEPostProcess(object):
    ...
    def __call__(self, outs_dict, shape_list):
        ...
        kernels = (pred.numpy() > self.thresh).astype('float32')
        text_mask = kernels[:, 0, :, :]
        text_mask = torch.unsqueeze(torch.Tensor(text_mask), axis=1)

        kernels[:, 0:, :, :] = torch.Tensor(kernels[:, 0:, :, :]) * text_mask
