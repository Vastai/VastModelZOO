...
def main(args):
    ...
    input_names = ["input"]
    output_names = ["output"]
    inputs = torch.randn(1, 3, 736, 736).cuda()

    torch_out = torch.onnx._export(model, inputs, os.path.join('onnx', args.checkpoint.split('/')[-1].replace('pth', 'onnx')), export_params=True, verbose=False,
