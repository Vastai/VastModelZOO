...
class Detector:
    ...
    def post_processing(self, class_list, input_id, height, width, stream_output_list):
        ...
        for i in range(classes):
            ...
            box_xy = box_boxes[i]
            # print('box_xy:\n', box_xy)
            box_xy[0] = box_xy[0] / self.model_size * width 
            box_xy[0] = np.clip(box_xy[0], 0, width)
            box_xy[2] = box_xy[2] / self.model_size * width
            box_xy[2] = np.clip(box_xy[2], 0, width) 
            box_xy[1] = box_xy[1] / self.model_size * height 
            box_xy[1] = np.clip(box_xy[1], 0, height)
            box_xy[3] = box_xy[3] / self.model_size * height 
            box_xy[3] = np.clip(box_xy[3], 0, height)
