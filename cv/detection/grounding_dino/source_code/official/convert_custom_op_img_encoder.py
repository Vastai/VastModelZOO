...
def convert_window_partition(graph):
            ...
            if flag:
                if count == 0 and len(cur_tensor_info.shape) == 3:
                    ...
                    if cur_tensor_info.shape[-2] == window_size and cur_tensor_info.shape[-4] == window_size:
                        match = True
                        H = cur_tensor_info.shape[-4] * cur_tensor_info.shape[-5]
                        W = cur_tensor_info.shape[-2] * cur_tensor_info.shape[-3]
    ...
    return graph
    # print(graph)
    # onnx.save(gs.export_onnx(graph), "swin_base_patch4_window7_224_sim_custom_op/swin_base_patch4_window7_224_sim_custom_op.onnx", save_as_external_data=True, all_tensors_to_one_file=True)
        ...
        for op_name in op_list:
            ...
            if flag:
                if count == 0 and len(cur_tensor_info.shape) == 4:
                    ...
                    if 1 == cur_tensor_info.shape[0] and cur_tensor_info.shape[1] > 1: # and ("vacc_roll" in cur_tensor_info.inputs[0].name)
                        match = True
...
def convert_patch_merging(graph):
    ...
    for key, tensor_info in tmap.items():
        ...
        for op_name in op_list:
            ...
            if flag:
                if count == 0 and len(cur_tensor_info.shape) == 4:
                    if 1 == cur_tensor_info.shape[0]  and cur_tensor_info.shape[1] > 1: # and cur_tensor_info.shape[1] == cur_tensor_info.shape[2]
                        match = True
                        H = cur_tensor_info.shape[1]
                        W = cur_tensor_info.shape[2]
                        C = cur_tensor_info.shape[3]
    ...
    for node in graph.nodes:
        ...
        if node.outputs[0].name in in_out_map.keys():
            ...
            global patch_merging_num
            patch_merging_num = len(custom_op_map)
            custom_op_map[wr_out_name] = custom_op[0]
        else:
            for i in range(len(node.inputs)):
                if node.inputs[i].name in custom_op_map.keys():
...
def convert_patch_merging_(graph):
    tmap = graph.tensors()
    op_list = ["Concat", "Slice", "Pad"]
        ...
        for op_name in op_list:
            ...
            if flag:
                if count == 0 and len(cur_tensor_info.shape) == 4:
                    ...
                    match = False
                    break
                if("Pad" != cur_tensor_info.inputs[0].op):
                    cur_tensor_info = pre_op_in_tensor
            else:
                match = False
                break
    ...
    for node in graph.nodes:
        ...
        if node.outputs[0].name in in_out_map.keys():
            ...
            global patch_merging_num
            patch_merging_num +=1
            custom_op = graph.vastai_patch_merging(reshape_op0[0], H=H, W=W, C=C, output_name=f"onnx::vastai_patch_merge_{patch_merging_num}")
            custom_op_map[wr_out_name] = custom_op[0]
        else:
            for i in range(len(node.inputs)):
                if node.inputs[i].name in custom_op_map.keys():
...
# p_m_in.append("/backbone/backbone.0/layers.2/downsample/Pad_output_0")
# p_m_out.append("/backbone/backbone.0/layers.2/downsample/vacc_patch_merge/Concat_output_0")
...
#     # for node in graph.nodes:
#     #     if node.op == "patch_merge":
#     #         input_name_map[node.inputs[0].name] = node.outputs[0].name
#     #         output_name_map[node.outputs[0].name] = node.inputs[0].name
#     assert len(p_m_in) == len(p_m_out) 
#     for i in range (len(p_m_in)):
#         input_name_map[p_m_in[i]] = p_m_out[i]
#         output_name_map[p_m_out[i]] = p_m_in[i]

#     for node in graph.nodes:
#         # print(node.inputs[0].name)
#         if node.outputs[0].name in input_name_map.keys():
#             wr_out_name = input_name_map[node.outputs[0].name]
#             wr_out_shape = tmap[wr_out_name].shape
            
#             B, H, W, C = wr_out_shape

#             # custom_op = graph.vastai_patch_merging(node.outputs[0], H=H, W=W, C=C, output_name=f"onnx::vastai_patch_merge_{len(custom_op_map)}")
#             # custom_op_map[wr_out_name] = custom_op[0]

#             reshape_op0 = graph.reshape(node.outputs[0], [4*H*W, C // 4], f"{node.outputs[0].name}_reshape", f"{node.outputs[0].name}_reshape_0")
#             custom_op = graph.vastai_patch_merging(reshape_op0[0], H=H, W=W, C=C, output_name=f"onnx::vastai_patch_merge_2")
#             reshape_op1 = graph.reshape(custom_op[0], [B, H, W, C], f"{node.outputs[0].name}_reshape", f"{node.outputs[0].name}_reshape_1")
#             custom_op_map[wr_out_name] = reshape_op1[0]
#         elif node.inputs[0].name in output_name_map.keys():
#             node.inputs[0] = custom_op_map[node.inputs[0].name]
#     graph.cleanup().toposort()
#     return graph

def rewrite_swin_base_patch4_window7_224_onnx():
    onnx_path = "weights/img_encoder_groundingdino_swint_ogc.onnx"
    ...
    graph = convert_patch_merging_(graph)

    # Path(f'swin_base_patch4_window7_224_sim_custom_op').mkdir(parents=True, exist_ok=True)
    onnx.save(gs.export_onnx(graph), "weights/img_encoder_groundingdino_swint_ogc_custom_op.onnx", save_as_external_data=False, all_tensors_to_one_file=False)
