...
1. 将原始仓库中`yolov6/models/effidehead.py`中`Detect`类替换成如下代码
...
2. 执行`export`
    ...
    ```bash
    python deploy/ONNX/export_onnx.py --weights /source/tmp/VastModelZOO/cv/detection/yolov6/workspace/yolov6s.pt
    ```
...
- 推理参考：[detection.py](./build_in/vsx/yolov6_detection.py)
    - 配置模型路径和测试数据路径等参数

    ```
    python ../build_in/vsx/yolov6_detection.py \
        --file_path path/to/det_coco_val \
        --model_prefix_path deploy_weights/official_yolov6_fp16/mod \
...
1. 性能测试
    ...
    ```bash
    vamp -m deploy_weights/official_yolov6_fp16/mod --vdsp_params ../build_in/vdsp_params/official-yolov6_n-vdsp_params.json -i 1 p 1 -b 1 -d 0
    ```
...
2. 精度测试
    ...
    - vamp推理获取npz结果输出
    ```bash
    vamp -m deploy_weights/official_yolov6_fp16/mod \
