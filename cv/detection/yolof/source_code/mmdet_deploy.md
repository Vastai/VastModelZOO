...
1. runstream
    ...
    python ../build_in/vsx/python/yolof_vsx.py \
        ...
        --model_prefix_path deploy_weights/mmdet_yolof_run_stream_int8/mod \
...
1. 性能测试
    ```bash
    vamp -m deploy_weights/mmdet_yolof_run_stream_int8/mod --vdsp_params ../build_in/vdsp_params/mmdet-yolof-vdsp_params.json -i 1 -b 1 -d 0 -p 1
    ```
...
2. 精度测试
    ...
    - vamp推理，获取npz结果输出
    ```bash
    vamp -m deploy_weights/mmdet_yolof_run_stream_int8/mod --vdsp_params vdsp_params.json -i 1 -b 1 -d 0 -p 1 --datalist npz_datalist.txt --path_output npz_output
    ```
