...
- 推理参考：[detection.py](../common/vsx/detection.py)
    ...
    python ../../common/vsx/detection.py \
        ...
        --model_prefix_path deploy_weights/ultralytics_yolov8s_int8/mod \
...
1. 性能测试
    ...
    ```bash
    vamp -m deploy_weights/ultralytics_yolov8s_fp16/mod --vdsp_params ../build_in/vdsp_params/ultralytics-yolov8s-vdsp_params.json -i 1 p 1 -b 1 -d 0
    ```
...
2. 精度测试
    ...
    - vamp推理获取npz结果输出
    ```bash
    vamp -m deploy_weights/ultralytics_yolov8s_fp16/mod \
