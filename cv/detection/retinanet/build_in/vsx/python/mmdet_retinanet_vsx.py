...
from mmdet.anchors import Anchors
from mmdet.utils import BBoxTransform, ClipBoxes
from mmdet.util import non_max_suppression, scale_coords
...
parse.add_argument("--model_prefix_path", type=str,
                   default="./mmdet_retinanet_run_stream_fp16/mod", help="model info")
parse.add_argument(
    "--vdsp_params_info",
    type=str,
...
class Detector:
    ...
    def post_processing(self, class_list, input_id, height, width, stream_output_list):
        ...
        yolo1_layer = torch.Tensor(stream_ouput_data[0]).unsqueeze(0)
        yolo2_layer = torch.Tensor(stream_ouput_data[1]).unsqueeze(0)
        yolo3_layer = torch.Tensor(stream_ouput_data[2]).unsqueeze(0)
        yolo4_layer = torch.Tensor(stream_ouput_data[3]).unsqueeze(0)
        yolo5_layer = torch.Tensor(stream_ouput_data[4]).unsqueeze(0)
        yolo6_layer = torch.Tensor(stream_ouput_data[5]).unsqueeze(0)
        yolo7_layer = torch.Tensor(stream_ouput_data[6]).unsqueeze(0)
        yolo8_layer = torch.Tensor(stream_ouput_data[7]).unsqueeze(0)
        yolo9_layer = torch.Tensor(stream_ouput_data[8]).unsqueeze(0)
        yolo10_layer = torch.Tensor(stream_ouput_data[9]).unsqueeze(0)
        ...
        boxtrans = BBoxTransform(std=[1.0, 1.0, 1.0, 1.0])
        clipbox = ClipBoxes()
        anchors = Anchors()(img)
        # print(anchors.shape)
...
{'bbox_mAP': 0.349, 'bbox_mAP_50': 0.52, 'bbox_mAP_75': 0.384, 'bbox_mAP_s': 0.183, 'bbox_mAP_m': 0.397, 'bbox_mAP_l': 0.469, 'bbox_mAP_copypaste': '0.349 0.520 0.384 0.183 0.397 0.469'}
'''