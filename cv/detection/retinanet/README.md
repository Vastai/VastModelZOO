![](../../../images/cv/detection/retinanet/speed.png)
...
branch: master
commit: 180014ad5724de84fd16a42bef69e8f58faf9e4f
```
...
retinanet系列的预处理主要是对输入图片利用`letterbox`算子进行resize(1024x1024)，然后减均值除以方差即可，均值配置如下（不同来源存在差异，具体参考推理脚本）：
...
![](../../../images/cv/detection/retinanet/arch.png)
...
| retinanet_resnext50_32x4d_fpn   |[mlcommons](https://github.com/mlcommons/inference/tree/master/vision/classification_and_detection)|   53.4   |   38.5   |   -    |    -    |        800    |
...
[Open Images](https://storage.googleapis.com/openimages/web/download_v6.html)是由谷歌发布的一个开源图片数据集，在2022年10月份发布了最新的V7版本（此仓库使用的是V6）。这个版本的数据集包含了900多万张图片，都有类别标记。其中190多万张图片有非常精细的标注。
...
- [mlcommons](./source_code/mlcommons_deploy.md)