...
class RetinaNet(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        for key, value in head_outputs.items():
            for va in value:
                output_list.append(va)
        
        # transpose
        # output_list[0] = output_list[0].transpose(1, 3) # nchw [1,2376,100,100] -> nwhc [1,100,100,2376]
        
        # chunk split channel
        # new_output = torch.chunk(output_list[0], 2, dim=1)
        # output_list[0] = new_output[1]
        # output_list.insert(0, new_output[0])
        # new_output2 = torch.cat(new_output, dim=1)
        return output_list
        ...
        if self.training:
            ...
            # output_list = []
            # for key, value in split_head_outputs.items():
            #     for va in value:
            #         output_list.append(va)
            ...
            for index, box in enumerate(detections[0]['boxes']):
                score = detections[0]['scores'][index]
                if score < 0.5:
                    pass
                label = detections[0]['labels'][index]
                p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
                s = [int(label), float(score)]
                s = [str(b) for b in s]
                s = ' '.join(s)
                # fin.writelines(s + '\n')
                cv2.rectangle(origin_image, p1, p2, (0, 0, 255), thickness=2, lineType=cv2.LINE_AA)
            cv2.imwrite('2.res.png', origin_image)
