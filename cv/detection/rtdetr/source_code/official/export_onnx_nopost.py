...
import os 
import sys
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), './code/model_check/vit_custom_models/source/RT-DETR/rtdetr_pytorch'))
...
def main(args, ):
    ...
    # np.random.seed(0)
    # data = torch.from_numpy(np.random.randn(1, 3, 640, 640).astype(np.float32))
    # output = model(data)
    # pred_logits = output['pred_logits']
    # pred_boxes= output['pred_boxes']
    # pred_logits.detach().numpy().astype(np.float16).tofile('result/pred_logits_600.bin')
    # pred_boxes.detach().numpy().astype(np.float16).tofile('result/pred_boxes_600.bin')
    # exit()
    ...
    if args.simplify:
        ...
        # onnx.save(onnx_model_simplify, args.file_name,  save_as_external_data=True, all_tensors_to_one_file=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', '-r', type=str, default='model/rtdetr_r18vd_dec3_6x_coco_from_paddle.pth')
    parser.add_argument('--file-name', '-f', type=str, default='onnx_model_all/model.onnx')
    parser.add_argument('--check',  action='store_true', default=1,)
    parser.add_argument('--simplify',  action='store_true', default=1,)
