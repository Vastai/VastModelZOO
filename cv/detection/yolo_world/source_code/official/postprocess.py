...
def get_bbox_post_process(results, cfg_nms, cfg_max_per_img, with_nms=True):
    def batched_nms(boxes, scores, idxs, cfg_nms, cfg_max_per_img, class_agnostic=False):
    ...
    if with_nms and results['bboxes'].shape[0] > 0:
        ...
        det_bboxes, keep_idxs = batched_nms(bboxes, results['scores'], results['labels'], cfg_nms, cfg_max_per_img)
...
def get_postprocess_gen_grids(cls_scores, bbox_preds, num_classes = 1203, featmap_strides = [8, 16, 32]):
    ...
    def get_grid_priors(featmap_sizes, strides):
        ...
        def grid_priors(featmap_sizes, dtype='float32', device='cpu', with_stride=False):
            multi_level_priors = []
    ...
    featmap_sizes = [cls_score.shape[2:] for cls_score in cls_scores]   # [(160, 160), (80, 80), (40, 40)]
    mlvl_priors = get_grid_priors(featmap_sizes, featmap_strides)
    flatten_priors = torch.cat(mlvl_priors)

    mlvl_strides = [flatten_priors.new_full((featmap_size[0] * featmap_size[1] * num_base_priors, ), stride) for featmap_size, stride in zip(featmap_sizes, featmap_strides)]   #self.featmap_strides [8, 16, 32], 
    flatten_stride = torch.cat(mlvl_strides) 

    flatten_cls_scores = [cls_score.permute(0, 2, 3, 1).reshape(1, -1, num_classes)  for cls_score in cls_scores]
    ...
    flatten_decoded_bboxes = get_bbox_coder_decode(flatten_priors[None], flatten_bbox_preds, flatten_stride)
...
def get_postprocess(cls_scores, 
                    bbox_preds, 
                    ori_shape=(427, 640), 
    ...
    flatten_cls_scores, flatten_decoded_bboxes, flatten_objectness = get_postprocess_gen_grids(cls_scores, bbox_preds)

    scores, labels, keep_idxs = filter_scores_and_topk(flatten_cls_scores, score_thr, nms_pre)
    bboxes = flatten_decoded_bboxes[keep_idxs]

    bboxes -= torch.from_numpy(np.array([pad_param[2], pad_param[0], pad_param[2], pad_param[0]]))
