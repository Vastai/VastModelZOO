...
from source_code import utils as utils
...
parse.add_argument("--model_prefix_path", type=str,
                   default="/path/to/yolofastv2/mod", help="model info")
parse.add_argument(
    "--vdsp_params_info",
    type=str,
...
class Detector:
    ...
    def post_processing(self, class_list, input_id, height, width, stream_output_list):
        ...
        yolo1_layer = torch.Tensor(stream_ouput_data[0].reshape(1, 12, 22, 22))
        yolo2_layer = torch.Tensor(stream_ouput_data[1].reshape(1, 3, 22, 22))
        yolo3_layer = torch.Tensor(stream_ouput_data[2].reshape(1, 80, 22, 22))
        yolo4_layer = torch.Tensor(stream_ouput_data[3].reshape(1, 12, 11, 11))
        yolo5_layer = torch.Tensor(stream_ouput_data[4].reshape(1, 3, 11, 11))
        yolo6_layer = torch.Tensor(stream_ouput_data[5].reshape(1, 80, 11, 11))
        
        preds = (yolo1_layer, yolo2_layer, yolo3_layer, yolo4_layer, yolo5_layer, yolo6_layer)
        output = utils.handel_preds(preds, self.model_size, self.model_size, torch.device('cpu'))
        output_boxes = utils.non_max_suppression(output)
        # print(output_boxes)

        scale_w = width / self.model_size
        scale_h = height / self.model_size

        for box in output_boxes[0]:
            box = box.tolist()
            cls_conf = box[4]
            cls_id = int(box[5])
            bb = [box[0] * scale_w, box[1] * scale_h, (box[2]-box[0]) * scale_w, (box[3]-box[1]) * scale_h]
    ...
    def run_sync(self, image: Union[str, np.ndarray]):
        ...
        # save_activation_value("runstream_yolo4_layer.linear.float", yolo4_layer)
        # save_activation_value("runstream_yolo5_layer.linear.float", yolo5_layer)
        # save_activation_value("runstream_yolo6_layer.linear.float", yolo6_layer)
        ...
        scale_h, scale_w = h / float(self.model_size), w / float(self.model_size)
