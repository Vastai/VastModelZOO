...
def make_grid(h, w, anchor_num, device):
    hv, wv = torch.meshgrid([torch.arange(h), torch.arange(w)])
    return torch.stack((wv, hv), 2).repeat(1,1,3).reshape(h, w, anchor_num, -1).to(device)

#特征图后处理
def handel_preds(preds, model_width, model_height, device):
    anchor_num = 3
    ...
    for i in range(len(preds) // 3):
        ...
        for r, o, c in zip(reg_preds, obj_preds, cls_preds):
            r = r.permute(1, 2, 0)
            r = r.reshape(r.shape[0], r.shape[1], anchor_num, -1)

            o = o.permute(1, 2, 0)
            o = o.reshape(o.shape[0], o.shape[1], anchor_num, -1)
            ...
            #计算anchor box的cx, cy
            grid = make_grid(r.shape[0], r.shape[1], anchor_num, device)
            stride = model_height /  r.shape[0]
