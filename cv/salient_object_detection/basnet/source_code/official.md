...
### step.3 模型转换
1. 根据具体模型，修改编译配置
    - [official_basnet.yaml](../build_in/build/official_basnet.yaml)
...
2. 模型编译
    ...
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/official_basnet.yaml
    ```
...
1. runstream
    ...
    python ../build_in/vsx/python/salod_vsx_inference.py \
        ...
        --model_prefix_path deploy_weights/official_basnet_run_stream_fp16/mod \
        --vdsp_params_info ../build_in/vdsp_params/official-basnet-vdsp_params.json \
        --save_dir ./runstream_output \
        --device 0
    ```

    - 精度统计，基于[eval.py](../../common/eval/eval.py)
        ```
    ...
    |-----------|-------|---------------|---------------|---------------|----------------|----------------|-------------|-------------|---------|---------|---------|-------|-------|
    | Method1   | 0.108 |         0.881 |         0.797 |         0.844 |          0.922 |          0.863 |           1 |       0.819 |   0.917 |   0.841 |   0.894 | 0.853 | 0.668 |
    ...
    | methods   |   mae |   maxfmeasure |   avgfmeasure |   adpfmeasure |   maxprecision |   avgprecision |   maxrecall |   avgrecall |   maxem |   avgem |   adpem |    sm |   wfm |
    |-----------|-------|---------------|---------------|---------------|----------------|----------------|-------------|-------------|---------|---------|---------|-------|-------|
    | Method1   | 0.098 |          0.91 |         0.821 |         0.869 |          0.947 |          0.884 |           1 |       0.844 |   0.933 |   0.856 |   0.909 | 0.872 | 0.689 |
...
### step.5 性能精度测试
1. 性能测试
    - 配置vdsp参数[official-basnet-vdsp_params.json](../build_in/vdsp_params/official-basnet-vdsp_params.json)
    ```bash
    vamp -m deploy_weights/official_basnet_run_stream_fp16/mod \
    --vdsp_params ../build_in/vdsp_params/official-basnet-vdsp_params.json \
    -i 1 p 1 -b 1
    ```
...
2. 精度测试
    ...
    - vamp推理得到npz结果
    ```bash
    vamp -m deploy_weights/official_basnet_run_stream_fp16/basnet \
        --vdsp_params ../build_in/vdsp_params/official-basnet-vdsp_params.json \
        -i 1 p 1 -b 1 \
    ...
    - 解析npz结果, 参考：[official_vamp_eval.py](../build_in/vdsp_params/official_vamp_eval.py)，
    ```bash
