...
### step.3 模型转换
1. 根据具体模型，修改编译配置
    - [official_dhsnet.yaml](../build_in/build/official_dhsnet.yaml)
...
2. 模型编译
    ...
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/official_dhsnet.yaml
    ```
...
1. runstream
    ...
    python ../build_in/vsx/python/salod_vsx_inference.py \
        ...
        --model_prefix_path deploy_weights/official_dhsnet_run_stream_fp16/mod \
        --vdsp_params_info ../build_in/vdsp_params/salod-dhsnet-vdsp_params.json \
    ...
    |-----------|-------|---------------|---------------|---------------|----------------|----------------|-------------|-------------|---------|---------|---------|-------|-------|
    | Method1   | 0.077 |         0.836 |         0.826 |         0.837 |          0.923 |          0.864 |           1 |       0.822 |   0.883 |   0.874 |   0.889 | 0.842 | 0.797 |
    ...
    | methods   |   mae |   maxfmeasure |   avgfmeasure |   adpfmeasure |   maxprecision |   avgprecision |   maxrecall |   avgrecall |   maxem |   avgem |   adpem |    sm |   wfm |
    |-----------|-------|---------------|---------------|---------------|----------------|----------------|-------------|-------------|---------|---------|---------|-------|-------|
    | Method1   | 0.071 |         0.864 |         0.848 |         0.858 |          0.949 |          0.874 |           1 |       0.867 |   0.898 |    0.89 |   0.903 | 0.861 | 0.821 |

    </details>

### step.5 性能精度测试
1. 性能测试
    - 配置vdsp参数[salod-dhsnet-vdsp_params.json](../build_in/vdsp_params/salod-dhsnet-vdsp_params.json)
    ```bash
    vamp -m deploy_weights/official_dhsnet_run_stream_int8/mod \
        --vdsp_params ../build_in/vdsp_params/salod-dhsnet-vdsp_params.json \
        -i 1 p 1 -b 1
    ```
