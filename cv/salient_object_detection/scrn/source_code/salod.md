...
### step.1 获取预训练模型
- 观察原仓库[test.py#L34](https://github.com/moothes/SALOD/blob/master/test.py#L34)，模型forward后只使用到一个字典的`['final']`值。为减少模型推理时的数据拷贝，修改模型返回值[methods/scrn.py#L219](https://github.com/moothes/SALOD/blob/master/methods/scrn.py#L219)，只返回一个值，`return pred_s`
...
### step.3 模型转换
1. 根据具体模型，修改编译配置
    - [official_scrn.yaml](../build_in/build/official_scrn.yaml)
...
2. 模型编译
    ...
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/official_scrn.yaml
    ```
...
1. runstream
    ...
    python ../build_in/vsx/python/salod_vsx_inference.py \
        ...
        --model_prefix_path deploy_weights/official_scrn_run_stream_fp16/mod \
        --vdsp_params_info ../build_in/vdsp_params/salod-scrn-vdsp_params.json \
    ...
    |-----------|-------|---------------|---------------|---------------|----------------|----------------|-------------|-------------|---------|---------|---------|-------|-------|
    | Method1   |  0.18 |          0.68 |         0.627 |         0.638 |          0.722 |          0.615 |           1 |        0.79 |   0.793 |    0.75 |   0.772 | 0.681 | 0.582 |
    ...
    | methods   |   mae |   maxfmeasure |   avgfmeasure |   adpfmeasure |   maxprecision |   avgprecision |   maxrecall |   avgrecall |   maxem |   avgem |   adpem |    sm |   wfm |
    |-----------|-------|---------------|---------------|---------------|----------------|----------------|-------------|-------------|---------|---------|---------|-------|-------|
    | Method1   | 0.153 |         0.762 |         0.684 |         0.703 |          0.812 |          0.668 |           1 |       0.868 |   0.838 |    0.78 |   0.812 | 0.734 | 0.639 |

    </details>

### step.5 性能精度测试
1. 性能测试
    - 配置vdsp参数[salod-scrn-vdsp_params.json](../build_in/vdsp_params/salod-scrn-vdsp_params.json)
    ```bash
    vamp -m deploy_weights/official_scrn_run_stream_int8/mod \
    --vdsp_params ../build_in/vdsp_params/salod-scrn-vdsp_params.json \
    -i 1 p 1 -b 1
    ```
...
2. 精度测试
    ...
    python ../../common/utils/image2npz.py \
        --dataset_path /path/to/sod/ECSSD/image \
        --target_path /path/to/sod/ECSSD/image_npz \
