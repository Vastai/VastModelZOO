...
def set_config():
    ...
    parse.add_argument(
        "--lr_image_dir",
        type=str,
        default="UIE-dataset/UIEBD/test/image",
        help="lr_image_dir img dir",
    )
    parse.add_argument(
        "--hr_image_dir",
        type=str,
        default="UIE-dataset/UIEBD/test/label",
        help="hr_image_dir img dir",
    )
    parse.add_argument(
        "--model_prefix_path",
        type=str,
        default="deploy_weights/UIEBD_final_sim-int8-kl_divergence-1_3_720_1280-vacc/mod",
        help="model info")
    parse.add_argument(
        "--vdsp_params_info",
        type=str,
        default="vacc_code/vdsp_params/official-usuir-vdsp_params.json",
        help="vdsp op info",
    )
...
if __name__ == '__main__':
    ...
    #############################################################################
    # import onnxruntime
    # session = onnxruntime.InferenceSession("final_weight/UIEBD_final_sim.onnx")
    # import torch
    ...
    for (image_path, result) in tzip(image_files, results):
        ...
        # image = cv2.resize(image_lr, (1280, 720))
...
"""
UIEBD_final_sim-fp16-none-1_3_720_1280-vacc
mean psnr: 20.40133148119128, mean ssim: 0.8552892947661273

UIEBD_final_sim-int8-kl_divergence-1_3_720_1280-vacc
mean psnr: 20.3860545935244, mean ssim: 0.8476951472417624
"""