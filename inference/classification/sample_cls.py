...
parse.add_argument('--file_path', type=str, help='img or dir  path')
parse.add_argument(
    '--model_info',
    type=str,
    default='classification/resnet/model_info/model_info_resnet.json',
    help='model info')
parse.add_argument(
    '--vdsp_params_info',
    type=str,
    default='classification/resnet/model_info/vdsp_params_resnet_rgb.json',
    help='vdsp op info')
parse.add_argument('--label_txt', type=str, help='label txt')
...
if args.task == 'run':
    ...
    for k in range(args.topk):
        print('{}: Relay top-{} id: {}, prob: {:.8f}, class name: {}'.format(
...
# benchmark
    ...
    for (image, result) in zip(images, results):
        ...
        for k in range(args.topk):
            fin.write(image + ': ' +
                      'Relay top-{} id: {}, prob: {:.8f}, class name: {}'.
                      format(k, cls_id[k], score_list[k], label_list[k]) +
                      '\n')
    time_end = time.time()
    print(
