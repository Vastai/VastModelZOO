...
parse.add_argument('--model_info',
                   type=str,
                   default='./info/model_info_yolov3.json',
                   help='model info')
parse.add_argument(
    '--vdsp_params_info',
    type=str,
...
def save_result(image_path, result, save_dir):
    ...
    for out_index in range(len(result[0])):
        ...
        y = int(result[2][out_index][1]) - 15 if int(
            result[2][out_index][1]) - 15 > 15 else int(
                result[2][out_index][1]) + 15
        cv.putText(
            origin_img,
            text,
