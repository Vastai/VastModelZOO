...
class BertChineseNER(Bert):
    def __init__(self,
                 ...
                 task_name: str = 'ner',
                 **kwargs):
        super(BertChineseNER, self).__init__(model_info, bytes_size, device_id,
                                             batch_size)
...
if __name__ == '__main__':
    ...
    parse.add_argument(
        ...
        help=
        'the data directory of the type of bytes to be predicted, the data type is .npz',
    )
    parse.add_argument('--batch_size', type=int, default=1, help='bacth size')
    parse.add_argument('--save_dir',
                       type=str,
                       default='output',
                       help='save the result path')
    args = parse.parse_args()

    # create bert ner model
    bert_ner = BertChineseNER(model_info=args.model_info,
