...
1. 根据step.3配置模型三件套信息，[model_info](./vacc_code/model_info/model_info_senet.json)
2. 配置数据预处理流程vdsp_params参数
   - [mmcls](./vacc_code/vdsp_params/sdk1.0/mmcls-seresnet50-vdsp_params.json)
   - [timm](./vacc_code/vdsp_params/sdk1.0/timm-legacy_senet154-vdsp_params.json)
   - [ppcls](./vacc_code/vdsp_params/sdk1.0/ppcls-senet154-vdsp_params.json)

3. 执行推理，参考[runstream](../common/sdk1.0/sample_cls.py)
    ```bash
    python ../common/sdk1.0/sample_cls.py --save_dir output/senet_result.txt
    ```

4. 精度评估
   ```bash
    python ../common/eval/eval_topk.py output/senet_result.txt
   ```
...
2. 性能测试
    ```bash
    ./vamp -m seresnet50-int8-max-3_224_224-vacc/seresnet50 --vdsp_params ./vacc_code/vdsp_params/vamp/timm-seresnet50-vdsp_params.json  -i 8 -p 1 -b 22
    ```
...
3. 获取精度信息
    ```bash
    ./vamp -m seresnet50-int8-max-3_224_224-vacc/seresnet50 --vdsp_params ./vacc_code/vdsp_params/vamp/timm-seresnet50-vdsp_params.json  -i 8 -p 1 -b 22 --datalist npz_datalist.txt --path_output output
    ```
4. 结果解析及精度评估
   ```bash
