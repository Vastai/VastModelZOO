...
def generate_random_image(width, height) -> Image.Image:
    np.random.seed(int((time.time() * 1e9) % (2**32 - 1)))
    random_data = np.random.randint(0, 256, (height, width, 3), dtype=np.uint8)
    return Image.fromarray(random_data, mode="RGB")
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in range(num_requests):
            ...
            temp_tokens = tokenizer.encode(prompt)
            while len(temp_tokens) > total_input_len:
                diff = len(temp_tokens) - total_input_len
                prompt = prompt[:-diff]
                temp_tokens = tokenizer.encode(prompt)
            total_input_len = len(temp_tokens)
            requests.append(
                SampleRequest(
                    prompt=prompt,
...
# -----------------------------------------------------------------------------
# RandomVision Dataset Implementation
# -----------------------------------------------------------------------------


class RandomVisionDataset(BenchmarkDataset):
    # Default values copied from benchmark_serving.py for the random dataset.
    DEFAULT_PREFIX_LEN = 0
    DEFAULT_RANGE_RATIO = 1.0
    DEFAULT_INPUT_LEN = 1024
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in range(num_requests):
            ...
            # create random image
            rand_img = generate_random_image(image_width, image_height)
            mm_content = process_image(rand_img)
            requests.append(
                SampleRequest(
                    prompt=prompt,
                    prompt_len=total_input_len,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.dataset_path != self.VISION_ARENA_DATASET_PATH:
            raise ValueError(
                f"Only support Vision Arena dataset.\
