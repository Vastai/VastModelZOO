...
from benchmark_dataset import RandomDataset, SampleRequest, RandomVisionDataset
...
def get_tp_from_v1_models(base_url):
    client = OpenAI(api_key="YOUR_API_KEY", base_url=base_url + "/v1")
    return client.models.list().data[0].tp
...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            vits.append(outputs[i].vit)
            prefills.append(outputs[i].prefill_us * 0.001)
            completed += 1
        else:
    ...
    metrics = BenchmarkMetrics(
        ...
        * 1000,  # ttfts is empty if streaming is not supported by backend
        ...
        mean_vit_ms=np.mean(vits) * 1000,
        mean_prefill_ms=np.mean(prefills),
    )
...
async def benchmark(
    ...
    if gpu_list is not None:
        vaml_monitor = monitor_manage.VAML_MonitorManager(
            cpu_list=cpu_list, gpu_list=gpu_list
        )
    ...
    if vaml_monitor:
        ...
        cpu_use_list = (
            vaml_monitor.monitor.cpu_use
        )  # 假设 vaml_monitor 是 self，并且 monitor 是其属性
        if cpu_use_list:
    ...
    print(
        "{:<40} {:<10.2f}".format(
            "Decode Token throughput (tok/s):",
            1000 / getattr(metrics, "mean_tpot_ms") * max_concurrency,
        )
    )
    print(
        "{:<40} {:<10.2f}".format(
            "Per-req Decoding token throughput (tok/s):",
            1000 / getattr(metrics, "mean_tpot_ms"),
        )
    )
    result = {
    ...
    model_tp = get_tp_from_v1_models(base_url)
    ...
    data = [
        {
            ...
            "Maximum req": max_concurrency,
            "Duration (s)": benchmark_duration,
            "Successful req": metrics.completed,
            "input tokens": metrics.total_input / metrics.completed,
            "generated tokens": metrics.total_output / metrics.completed,
            "Req throughput (req/s)": metrics.request_throughput,
            "Output token throughput (tok/s)": metrics.output_throughput,
            "Total token throughput (tok/s)": metrics.total_token_throughput,
        }
    ]
    if metrics.mean_vit_ms > 0.1:
        data[0].update({"Mean VIT (ms)": metrics.mean_vit_ms})
    data[0].update(
        {
            "Mean prefill (ms)": metrics.mean_prefill_ms,
            "Mean TTFT (ms)": getattr(metrics, "mean_ttft_ms"),
            "Mean TPOT (ms)": getattr(metrics, "mean_tpot_ms"),
            "Decode Token throughput (tok/s)": 1000
            / getattr(metrics, "mean_tpot_ms")
            * max_concurrency,
            "Per-req Decoding token throughput (tok/s)": 1000
            / getattr(metrics, "mean_tpot_ms"),
        }
    )
    ...
    save_to_csv(data, f"benchmark_{model_name}.csv")
    return result
...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "sonnet":
        ...
        dataset_class = (
            ...
            and args.hf_subset is None
            else HuggingFaceDataset
        )
        ...
        input_requests = RandomDataset(dataset_path=args.dataset_path).sample(
            tokenizer=tokenizer,
            num_requests=args.num_prompts,
        ...
        prompt_template_name = "raw"
    elif args.dataset_name == "random_vision":
        import ast

        width, height = ast.literal_eval(args.random_image_size)
        input_requests = RandomVisionDataset(dataset_path=args.dataset_path).sample(
            tokenizer=tokenizer,
            num_requests=args.num_prompts,
            prefix_len=args.random_prefix_len,
            input_len=args.random_input_len,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="random",
        choices=["sharegpt", "burstgpt", "sonnet", "random", "hf", "random_vision"],
    ...
    random_group.add_argument(
        ...
        type=str,
        default="[1280,720]",
        help="Image size:[width,height] created with random number",
    )
    random_group.add_argument(
        "--random-output-len",
        type=int,
