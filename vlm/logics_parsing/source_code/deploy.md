...
1. 下载模型权重
    ...
    | models  |tips |
    | :---: | :--: |
    | [Logics-MLLM/Logics-Parsing](https://huggingface.co/Logics-MLLM/Logics-Parsing) |[modeling_qwen2_5_vl_vacc.py](./modify_modeling/modeling_qwen2_5_vl_vacc.py) |
    

2. 网络修改
    - 为了方便部署`Logics-Parsing`系列模型，在官方源码的基础上，需要对`modeling_qwen2_5_vl.py`进行适当修改
...
### step.3 模型转换
1. 根据具体模型修改配置文件：
    - [logics_parsing_llm.yaml](../build_in/build/logics_parsing_llm.yaml)
    - [logics_parsing_visual.yaml](../build_in/build/logics_parsing_visual.yaml)

    ```bash
    cd logics_parsing
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/logics_parsing_llm.yaml
    vamc compile ../build_in/build/logics_parsing_visual.yaml
    ```
...
--vit_model vamc_results/logics_parsing_visual_fp16 \
--port 9900 \
--llm_devices "[4,5,6,7]" \
--vit_devices "[3]" \
--min_pixels 78400 \
...
- input_ids转为embedding部分，需要使用从预训练模型获取的embedding权重：`emb_tokens.bin`
    ...
    model_name = "./Logics-MLLM/Logics-Parsing"
