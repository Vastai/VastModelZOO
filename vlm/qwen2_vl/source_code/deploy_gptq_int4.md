...
1. 下载模型权重
    ...
    | models  | tips |
    | :---: | :--: |
    | [Qwen/Qwen2-VL-7B-Instruct-GPTQ-Int4](https://huggingface.co/Qwen/Qwen2-VL-7B-Instruct-GPTQ-Int4) |[modeling_qwen2_vl_vacc.py](./modify_modeling/modeling_qwen2_vl_vacc.py) |
    

2. 网络修改
    - 为了方便部署`Qwen2-VL`系列模型，在官方源码的基础上，需要对`modeling_qwen2_vl.py`进行适当修改
    - 修改后：[modeling_qwen2_vl_vacc.py](./modify_modeling/modeling_qwen2_vl_vacc.py)
...
1. visual模块根据具体模型修改配置文件：
    - [qwen2_vl_visual_gptq.yaml](../build_in/build/qwen2_vl_visual_gptq.yaml)

    ```bash
    cd qwen2_vl
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/qwen2_vl_visual_gptq.yaml
    ```
3. llm模块抽取：
    - 修改相应权重路径并执行 [contract_llm.py](../source_code/contract_llm/contract_llm.py)

    ```bash
    python3 ../source_code/contract_llm/contract_llm.py
    ```
...
```bash
vastgenx serve --model Qwen2-VL-7B-Instruct-GPTQ-Int4-llm \
--vit_model vamc_results/qwen2_vl_7b_visual_gptq_fp16 \
--port 9900 \
--llm_devices "[0,1]" \
...
- 仅验证了Qwen2-VL-7B-Instruct-GPTQ-Int4，transformers版本需要较新
- 精度测试可测试多个数据集：
    - MMBench_DEV_EN
    - MMBench_DEV_CN
    - MMMU_DEV_VAL
...
- input_ids转为embedding部分，需要使用从预训练模型获取的embedding权重：`embed_tokens.bin`
    ...
    model_name = "./vlm/Qwen/Qwen2-VL-7B-Instruct-GPTQ-Int4"
...
- 抽取llm模块得到的Qwen2-VL-7B-Instruct-GPTQ-Int4-llm 所对应的 `config.json`，需要进行适当修改以满足llm模块，修改后的：[config_int4_llm.json](./modify_modeling/config_int4_llm.json)
