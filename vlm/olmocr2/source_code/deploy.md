...
1. 下载模型权重
   ...
   | models  | tips |
    | :---: | :--: |
    | [allenai/olmOCR-2-7B-1025](https://hf-mirror.com/allenai/olmOCR-2-7B-1025) |[modeling_qwen2_5_vl_vacc.py](./modify_modeling/modeling_qwen2_5_vl_vacc.py) |
    

2. 网络修改
    - 为了方便部署`olmOCR-2-7B-1025`系列模型，在官方源码的基础上，需要对`modeling_qwen2_5_vl.py`进行适当修改
...
### step.3 模型转换
1. 根据具体模型修改配置文件：
    - [olmocr2_llm.yaml](../build_in/build/olmocr2_llm.yaml)
    - [olmocr2_visual.yaml](../build_in/build/olmocr2_visual.yaml)

    ```bash
    cd olmocr2
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/olmocr2_llm.yaml
    vamc compile ../build_in/build/olmocr2_visual.yaml
    ```
...
- input_ids转为embedding部分，需要使用从预训练模型获取的embedding权重：`emb_tokens.bin`
    ...
    model_name = "./vlm/allenai/olmOCR-2-7B-1025"
