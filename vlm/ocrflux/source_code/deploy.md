...
1. 下载模型权重
    ...
    | models  | tips |
    | :---: | :--: |
    | [ChatDOC/OCRFlux-3B](https://hf-mirror.com/ChatDOC/OCRFlux-3B)| [modeling_qwen2_5_vl_vacc.py](./modify_modeling/modeling_qwen2_5_vl_vacc.py) |
    

2. 网络修改
    - 为了方便部署`OCRFlux`系列模型，在官方源码的基础上，需要对`modeling_qwen2_5_vl.py`进行适当修改
...
### step.3 模型转换
1. 根据具体模型修改配置文件：
    - [ocrflux_llm.yaml](../build_in/build/ocrflux_llm.yaml)
    - [ocrflux_visual.yaml](../build_in/build/ocrflux_visual.yaml)

    ```bash
    cd ocrflux
    mkdir workspace
    cd workspace
    vamc compile ../build_in/build/ocrflux_llm.yaml
    vamc compile ../build_in/build/ocrflux_visual.yaml
    ```
...
- 本例参考：
```bash
vastgenx serve --model vamc_results/ocrflux_llm_fp16 \
--vit_model vamc_results/ocrflux_visual_fp16 \
--port 9900 \
--llm_devices "[4,5,6,7]" \
--vit_devices "[3]" \
--min_pixels 78400 \
