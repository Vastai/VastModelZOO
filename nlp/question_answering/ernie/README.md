...
## **Model Info**
- ernie模型在SQUAD1.1数据集上的新性能

| Model  | SQUAD 1.1 F1/EM |
| :---------------: | :--------: |
| ernie1.0_base_zh    | 75.5/64.1 |
| ernie2.0_base_en    | 91.2/84.2 |
| ernie2.0_large_en    | 93.3/86.9 |
| ernie3.0_base_zh    | 82.0/71.8 |
| ernie3.0_medium_zh    | 83.2/73.5 |
| ernie3.0_xbase_zh    | 87.7/79.8 |
</br>
...
### step.1 模型 finetune
-  huggingface，模型微调说明：[huggingface_ernie_squad.md](./huggingface/source_code/finetune/huggingface_ernie_squad.md)

### step.2 获取模型
- huggingface，预训练模型导出至torhcscript格式，说明: [pt2torchscript](./huggingface/source_code/pretrain_model/README.md)
...
> 注意： 由于 compiler 1.5 版本将 transformer 相关模型的输入改变为6个。 因此，1.5 版本的校验数据集需使用 `评估数据集-v1.5`。



### step.4 模型转换
1. 根据具体模型修改配置文件
   - [huggingface](./huggingface/build_in/build/ernie_qa.yaml)
...
2. 模型编译
    ...
    mkdir workspace
    cd workspace
    vamc build ../huggingface/build_in/build/ernie_qa.yaml
    vamc compile ../huggingface/build_in/build/ernie1.0_base_zh_qa-384.yaml
    ```


### step.5 模型推理

- 基于 [sequence2npz.py](../common/utils/sequence2npz.py)，获得推理数据`npz`以及对应的`npz_datalist.txt`（可选）
   ```bash
   python ../../common/utils/sequence2npz.py \
       --data_dir /path/to/datasets/SQuAD_1.1/val_npz/ \
       --save_dir /path/to/datasets/squad_1.1/
   ```
- 或基于 [gen_datalist.py](../common/utils/gen_datalist.py)，获得对应的 `npz_datalist.txt`（可选）

   ```bash
   python ../../common/utils/gen_datalist.py \
       --data_dir /path/to/datasets/SQuAD_1.1/val_npz_6input \
