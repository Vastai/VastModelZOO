...
> [huggingface](https://huggingface.co/aspire/acge_text_embedding)
...
| :----------------------------------------------------------: | :-------: | :-------------: | :----------------------------------------------------------: |
|      [aspire/acge_text_embedding](https://huggingface.co/aspire/acge_text_embedding)       |   1024    |      1024       | multilingua,  No need for "instruction"|
...
在模型加载的时候，可还以根据modules.json进行不同层的加载，本模型中除了上述的和关于BertModel不同层即Transformer的加载之外，还配置了Pooling和Normalize，Normalize具体的配置没有给出。当使用SentenceTransformer进行加载的时候，可以利用Pooling 或者 Normalize的配置，对输出进行不同的后处理。具体使用案例可以参考sentencetransformer Pooling。
