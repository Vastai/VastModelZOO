...
class EmbeddingX:
    def __init__(
        ...
        self.model_output_count = self.model.output_count
        self.current_id = -1
        self.input_dict = {}
        self.event_dict = {}
        self.result_dict = {}
    ...
    def __call__(self, inputs):
        ...
        vsx_outputs = self.infer_stream.run_sync(inputs)
        end_time = time.time()
        running_time = end_time - start_time
        # print("time cost:", float(end_time - start_time), "s")
        if self.model_output_count > 1:
            results = []
            for i in range(len(vsx_outputs)):
                outputs = []
                for j in range(len(vsx_outputs[i])):
                    outputs.append(vsx.as_numpy(vsx_outputs[i][j]))
