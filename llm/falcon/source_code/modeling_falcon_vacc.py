...
def local_scaled_dot_product_attention(Q, K, V, attn_mask=None, dropout_p=0.0, is_causal=False):
...
class FalconRotaryEmbedding(nn.Module):
    ...
    def cos_sin(self, seq_len: int, past_key_values_length: int, device="cpu", dtype=torch.bfloat16, position_ids=None) -> torch.Tensor:
        ...
        if position_ids is not None:
            if seq_len != 1:
                raise ValueError('wrong seq_len for iter1', seq_len)
            cos_cached = self.cos_cached.squeeze(1).squeeze(0)  #[1,1,total_length, head_size] -> [total_length, head_size]
            sin_cached = self.sin_cached.squeeze(1).squeeze(0)
            cos_cached = cos_cached[position_ids].unsqueeze(1) #[bs, 1, iter1_seq_length, head_size]
    ...
    def forward(self, query, key, past_key_values_length=0, position_ids=None):
        batch, _, seq_len, head_dim = query.shape  #torch.Size([1, 71, 1, 64])
        cos, sin = self.cos_sin(seq_len, past_key_values_length, query.device, query.dtype, position_ids)
        return (query * cos) + (rotate_half(query) * sin), (key * cos) + (rotate_half(key) * sin)


def _make_causal_mask_origin(
...
def _make_causal_mask(
    ...
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length).bool()
...
class FalconAttention(nn.Module):
    def __init__(self, config: FalconConfig):
        ...
        self.query_key_value = nn.Linear(self.hidden_size, qkv_out_dim, bias=config.bias)
    ...
    def _split_heads(self, fused_qkv: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.new_decoder_architecture:
            ...
            # print('multi_query000')
            batch_size, seq_length, three_times_hidden_size = fused_qkv.shape
            fused_qkv = fused_qkv.view(batch_size, seq_length, self.num_heads + 2, self.head_dim)
            # return fused_qkv[..., :-2, :], fused_qkv[..., [-2], :], fused_qkv[..., [-1], :]
            return fused_qkv[..., :-2, :], fused_qkv[..., -2, :].reshape(batch_size, seq_length, 1, self.head_dim), fused_qkv[..., -1, :].reshape(batch_size, seq_length, 1, self.head_dim)
    ...
    def forward(
        ...
        # if  value_layer.shape[0] == 1 and value_layer.shape[1] == 2048 and value_layer.shape[2] == 1 and value_layer.shape[3] == 64:
        #     bias = 0.0001*torch.ones(([64])).to(torch.float32)
        #     value_layer = value_layer + bias
        #     print('dummy bias')
        ...
        # )
        query_layer = query_layer.transpose(1, 2)#.reshape(batch_size * self.num_heads, query_length, self.head_dim)
        key_layer = key_layer.transpose(1, 2)   # torch.Size([batch_size, num_heads, query_length, self.head_dim])
        ...
        bs, _, kv_length, _ = key_layer.shape
        if use_cache:
        ...
        key_layer_ = key_layer
        value_layer_ = value_layer
        ...
        if alibi is None:
            if output_attentions:
                ...
                # )
                attn_output = local_scaled_dot_product_attention(
                    query_layer_, key_layer_, value_layer_, attention_mask_float, 0.0, is_causal=False
                )
...
class FalconDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        alibi: Optional[torch.Tensor],
        attention_mask: torch.Tensor,
        position_ids: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        layer_past: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        head_mask: Optional[torch.Tensor] = None,
        use_cache: bool = False,
        output_attentions: bool = False,
...
class FalconModel(FalconPreTrainedModel):
    def __init__(self, config: FalconConfig):
        ...
        self.insert_slcie = config.insert_slice if hasattr(config, "insert_slice") else 0
    ...
    def _prepare_attn_mask(
        ...
        if seq_length > 1:
            combined_attention_mask = _make_causal_mask(
                input_shape, torch.float32, device=device, past_key_values_length=past_key_values_length
            )
    ...
    def forward(
        ...
        if past_key_values is None:
            ...
            # past_key_values = self._convert_to_rw_cache(past_key_values)  #4->3
        ...
        if position_ids is None and past_key_values[0] is not None:
            device = input_ids.device if input_ids is not None else inputs_embeds.device
            position_ids = torch.arange(
                past_key_values_length, seq_length + past_key_values_length, dtype=torch.long, device=device
            )
            position_ids = position_ids.unsqueeze(0).view(-1, seq_length)
        elif past_key_values[0] is not None:
        ...
        # add strided_slice
        if not self.iter_num and self.insert_slcie:
