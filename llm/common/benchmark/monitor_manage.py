...
class MonitorManager:
    ...
    def monitor_cpu_usage(self, cpu_list=None, sampling_time=1):
        ...
        while self.cpu_sampling_flag:
            ...
            avg_cpu = sum([cpu_percents[i] for i in cpu_list_to_use]) / len(cpu_list_to_use) if cpu_list_to_use else psutil.cpu_percent(interval=sampling_time)
...
class VAML_MonitorManager:
    def __init__(self, cpu_list=None, gpu_list=None):
        self.cpu_list = cpu_list if cpu_list is not None else list(range(psutil.cpu_count()))
        ...
        self.tp_device_ids_list = [i for i in (gpu_list or all_gpu_ids) if i in all_gpu_ids]
        if self.monitor.vaml_status:
            ...
            logger.warning("VAML initialization failed. GPU monitoring features will be disabled.")
            self.model_tp_num = 0
    ...
    def start_profile(self):
        ...
        cpu_monitor_thread = threading.Thread(target=self.monitor.monitor_cpu_usage, args=(self.cpu_list,))
    ...
    def get_profile_summary(self):
        results_summary = {
            'oclk': 0, 'temperature': 0, 'power': 0, 'mem': 0, 'ai': 0,
            'temperature_mean': 0, 'temperature_median': 0, 'temperature_p99': 0,
            'power_mean': 0, 'power_median': 0, 'power_p99': 0,
            'ai_mean': 0, 'ai_median': 0, 'ai_p99': 0
        }
    ...
    def _calculate_median(self, data):
        ...
        n = len(sorted_data)
        return (sorted_data[n // 2 - 1] + sorted_data[n // 2]) / 2 if n % 2 == 0 else sorted_data[n // 2]
