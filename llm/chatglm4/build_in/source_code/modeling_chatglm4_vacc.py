...
class CoreAttention(torch.nn.Module):
    def __init__(self, config: ChatGLMConfig, layer_number):
        ...
        projection_size = config.kv_channels * (config.num_attention_heads // config.rank_num)

        # Per attention head and per partition values.
        self.hidden_size_per_partition = projection_size
        self.hidden_size_per_attention_head = projection_size // (config.num_attention_heads // config.rank_num)
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        if 0 and pytorch_major_version >= 2:
            attn_weights = torch.matmul(
                query_layer, key_layer.transpose(2, 3)) / math.sqrt(self.head_dim)
