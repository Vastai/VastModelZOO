...
def _gen_alibi_mask(tensor, n_head, max_pos, rankid = 0, rank_num = 1):
    head_per_rank = n_head // rank_num
    slopes = torch.Tensor(_get_interleave(n_head))
    position_point = torch.arange(max_pos) - max_pos + 1
    position_point = position_point.unsqueeze(0).unsqueeze(0).expand(head_per_rank, -1, -1)
    diag = torch.diag(position_point[0])
    position_point = position_point - diag.unsqueeze(0).unsqueeze(0).transpose(-1, -2)
    alibi = slopes[head_per_rank * rankid : head_per_rank * (rankid + 1)].unsqueeze(1).unsqueeze(1) * position_point
    alibi = alibi.view(head_per_rank, 1, max_pos)
...
class BaichuanAttention(torch.nn.Module):
    ...
    def forward(
        ...
        proj = proj.reshape(proj.shape[0], proj.shape[1], 3, -1)
...
class BaichuanModel(BaichuanPreTrainedModel):
    def __init__(self, config: BaichuanConfig):
        ...
        self.rank_num = config.rank_num if hasattr(config, "rank_num") else 1
        self.rank_index = config.rank_index if hasattr(config, "rank_index") else 0
    ...
    def get_alibi_mask(self, tensor, seq_length_with_past, rank_index):
        if self.training:
            ...
            real_n_head = math.ceil(self.n_head * 1.0 / rank_num) * rank_num
            if self.first_run:
                if rank_index == rank_num - 1:
                    self.first_run = False
                self.register_buffer(
                    "future_mask",
                    _gen_alibi_mask(tensor, real_n_head, self.max_cache_pos, rank_index, rank_num).to(
                        tensor
                    ),
                    persistent=False,
                )
        ...
        if self.training:
            if (
                ...
                self.alibi_mask = self.get_alibi_mask(
                    inputs_embeds, seq_length_with_past, call_cnt // 2
                )
            alibi_mask = self.alibi_mask
        else:
            alibi_mask = self.get_alibi_mask(inputs_embeds, seq_length_with_past, self.rank_index)
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    ...
    def quantize(self, bits: int):
        try:
            from .quantizer_vacc import quantize_online
        except ImportError:
