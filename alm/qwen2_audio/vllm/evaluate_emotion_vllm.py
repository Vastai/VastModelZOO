...
ds_collections = {
    "meld": {"path": "data/ser/meld_eval.jsonl"},
}
...
def main():
    ...
    for src, items in results_by_source.items():
        refs = [normalize_label(x["gt"]) for x in items]
        hyps = [normalize_label(x["response"]) for x in items]
        acc = accuracy_score(refs, hyps)
        line = f"source: {src}  cnt: {len(items)}  ACC_score: {acc:.4f}"
        print(line)
