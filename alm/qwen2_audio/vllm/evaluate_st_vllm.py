...
ds_collections = {
    "covost2": {"path": "data/st/covost2_eval.jsonl"},
}
...
def bleu_tokenize_from_source(source: str) -> str:
    text_lan = source.split("_")[-2] if "_" in source else "13a"
    if text_lan == "ja":
        return "ja-mecab"
    if text_lan == "zh":
        return "zh"
...
def main():
    ...
    append_lines(summary_path, header_lines)
    ...
    if total_audio_sec > 0:
        ...
        rtf_line = f"RTF: {rtf:.4f}  (infer_sec={total_infer_sec:.2f}, audio_sec={total_audio_sec:.2f}, speed={speed:.2f}x)"
        print(rtf_line)
    ...
    for src, items in results_by_source.items():
        ...
        hyps = [x["response"] for x in items]
        tok = bleu_tokenize_from_source(src)
        bleu = sacrebleu.corpus_bleu(hyps, [refs], tokenize=tok).score
        line = f"source: {src}  cnt: {len(items)}  bleu score: {bleu:.4f}  tokenize={tok}"
        print(line)
        bleu_lines.append(line)

    if bleu_lines:
        append_lines(summary_path, bleu_lines)

    print("saved ->", str(results_file))
    print("[Summary appended to]", str(summary_path))
